...
const input = document.querySelector(".fname1");
const stack=[];
// Animations Timeouts
...
handleEmptyListError();
    return new Promise(resolve => {
		...
		setTimeout(() => {
			...
			//adding the massage
			error.innerHTML = `Item ${itemValue} is Popped.`;
...
async function add(i, data) {
	...
	error.innerHTML=data+" "+"is inserted";
	box[0].innerHTML = stack[stack.length - 1];
...
for(let i=1;i<nodes.length;i++)
{
	...
	if(nodes[i].firstChild.innerHTML>max1)
	max1=nodes[i];

	if(nodes[i].firstChild.innerHTML<min1)
	min1=nodes[i];
}
...
document.writeln(min1);
output1.innerText = "MAX  :"+max1;
output2.innerText = "MIN  :"+min1;
}
